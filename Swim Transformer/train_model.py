...
def train_model(dataloader, valid_dataloader, in_dim, out_dim, batch_size, patch_size, depths, input_history, EPOCH, path_save_model, device):
    same_seeds(1234)
    device = torch.device(device)
    model = Swin(in_dim, out_dim, batch_size, device, patch_size, depths, input_history).to(device)
    print(model)
