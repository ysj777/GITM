...
def inference(in_dim, out_dim, test_dataloader, patch_size, depths, input_history, device, mode, val, val2):
    tec_tar, tec_pred = [], []
    model = Swin(in_dim, out_dim, 1, device, patch_size, depths, input_history).to(device)
    model.load_state_dict(torch.load('save_model/best_train_Swin2.pth'))
    model.eval()
    total_rmse, step = 0, 0
